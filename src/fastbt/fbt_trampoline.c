...
static void initialize_ret_trampolines(struct thread_local_data *tld) {
  ...
  *hitloc = (char)(((int32_t)transl_instr)-(((int32_t)hitloc)+1));
  ...
  /* BEGIN for SHADOWSTACK without DEBUG */
  BEGIN_ASM(transl_instr)  
    pushl %ebx
	...
	/* TODO: it seems that %ebx will contain the real return address and therefore tld->ind_target as well 
...
#if defined(SHADOWSTACK) 
#if (defined(SHADOWSTACK_DEBUG) || defined(REAUTH_DEBUG))
...
#endif  // SHADOWSTACK_DEBUG || REAUTH_DEBUG
...
static void* shadowstack_reauthenticate(struct thread_local_data *tld,
	...
	nr_of_reauths++;
	llprintf(
        "*** SHADOWSTACK FAILURE *** Return address should be %x (call target %x), but is %x.\n", 
        (ulong_t)tld->top_of_shadowstack->return_address, (ulong_t)tld->top_of_shadowstack->call_target,
        (ulong_t)new_target
    );
    
    llprintf("********** shadowstack print ********** :\n");
    fbt_shadowstack_print(tld);
    llprintf("******************************\n"); 
    
    llprintf("nr of reauths so far: %d\n", nr_of_reauths);
#endif
  ...
  if (esp == top->stack_pointer) {
    if (top->return_address != (ulong_t)new_target) {
      ...
      #ifdef ENABLE_UI_MESSAGES
	  notify_ui(UI_RET_VIOLATION, "Return address corruption detected!");
	  #endif
