...
/* TODO: verify if _dl_fini of ld-linux is functionaly equivalent with our implementation */
/**
 * This function is called by libc after the loaded program exited.
 * ld-linux has _dl_fini for this purpose. We need to call all loaded libraries' cleanup/fini functions.
 */
void cleanup() {
#ifdef D_LOAD
  ...
  dso *so;
	...
	fbt_print_all_callbacks();
#endif

#if defined(PRINT_CFTX_CHECK_TRANSFER_STATS) || defined(PRINT_CFTX_FASTLOOKUP_STATS)
  struct dso_chain *dso;
  if(dso_chain != NULL) {
	  sl_printf("\n[CFTX STATS] **********************************\n");
	  for (dso = dso_chain; dso->next != 0; dso = dso->next) {
			  #if defined(PRINT_CFTX_CHECK_TRANSFER_STATS) || defined(PRINT_CFTX_FASTLOOKUP_STATS)
			  sl_printf("* stats for %s\n", dso->name);
			  #endif
			  #if defined(PRINT_CFTX_CHECK_TRANSFER_STATS)
			  sl_printf(" * nr of check transfers: %d\n", dso->nr_check_transfers_total);
			  sl_printf(" * nr of check transfers on the slowpath: %d\n", dso->nr_check_transfers_slowpath);
			  #endif
			  #if defined(PRINT_CFTX_FASTLOOKUP_STATS)
			  sl_printf(" * nr of fastlookups: %d\n", dso->nr_fastlookups);
			  #endif
	  }
  }
#endif /* defined(PRINT_CFTX_CHECK_TRANSFER_STATS) || defined(PRINT_CFTX_FASTLOOKUP_STATS) */

#if defined(PRINT_CFTX_TYPE_STATS)
	fbt_print_cftx_type_stats();
#endif

  /* For every loaded DSO execute all fini functions except for the executable. */
 ...
 */
static void init_global_data(Elf32_auxv_t *auxv) {
  ...
  /* former version used sysinfo_int80, now use vDSO */
  //GL_RO(_dl_sysinfo) = (unsigned long)sysinfo_int80;
  GL_RO(_dl_sysinfo) = (uintptr_t)get_aux_value(auxv, AT_SYSINFO);
  GL_RO(_dl_sysinfo_dso) = (Elf32_Ehdr*)get_aux_value(auxv, AT_SYSINFO_EHDR);
...
static dso* move_libc_to_front(dso* st){
  ...
  while (1){
    ...
    if (sl_strncmp("libc.so", d->name, 7) == 0){
 ...
 */
int sl_main(int argc, char **argv, char **envp, Elf32_auxv_t *auxv) {
  ...
  if(filename == 0) {
	  /* Handle case where the command line path just consists of the binary name i.e. without '/' */
	  sl_strncpy(name, path, MAX_LIB_NAME);
	  path[0] = '\0';
  } else {
	  filename++;
  ...
  /* TODO: is this still required? watch out for the init_loader_dso() call within load_libdetox() */
  ...
  init_loader_dso((Elf32_Ehdr*)get_aux_value(auxv, AT_SYSINFO_EHDR));
#endif
...
/* Note that if ATTACH_LIBDETOX is defined then it is loaded before
  ...
  libc_malloc = resolve_symbol(0, "malloc", 0, 1, 1, &sym_found, &so_found, NULL);

  /* Save pointer to chain of loaded objects */
  GL(_dl_ns[0])._ns_loaded = (struct link_map *)so_chain;

/* TODO: probably not required but has to be verified */
  ...
  struct thread_local_data *tld = fbt_init(0);
  ...
  /* If it's a PIE executable -> adapt entry address */
  if(so_main->type != ET_EXEC) {
	  so_main->entry = (void*)((uintptr_t)so_main->entry + (uintptr_t)so_main->base_addr);
  }
...
/**
 * Main function of the RTLD.
 */
int sl_mainRTLD(int argc, char **argv, char **envp, Elf32_auxv_t *auxv) {
  ...
  /* Get path to executable */
  char *path = sl_malloc(MAX_PATH_LEN);
  sl_strncpy(path, argv[0], MAX_PATH_LEN);
  
  /* Open file */
  long fd = sl_open(path, O_RDONLY);
  if(fd == -1) {
    sl_printf("sl_mainRTLD: Error: could not open %s.\n", path);
    return 1;
  }
  ...
  /* Load executable and all dependent objects */
  dso *so_main = load_elf(0, name, path, fd, 0, 1, auxv);
...
/* TODO: handle different calling convention! It seems that libdl expects at the end a ret $0x8 instead of a plain ret! */
/* It seems that the glibc guys switched from cdecl calling convention to stdcall. */
long REG_ARGS_STDCALL(3) catch_error(const char **objname, const char **errstring,
...
/* TODO: handle different calling convention! It seems that libdl expects at the end a ret $0x8 instead of a plain ret! */
/* It seems that the glibc guys switched from cdecl calling convention to stdcall. */
void REG_ARGS_STDCALL(3) signal_error(long errcode, const char *objname,
  ...
  //sl_printf("Signal error: %s, %s, %p, %x, %x\n", objname, errstring, catch, GL(_dl_error_catch_tsd), libc_malloc);
...
void *dl_open(const char *file, long mode, const void *caller_dlopen,
  ...
  if(so_loaded->dso != 0) {
   UNPROT_DATA(so_loaded->dso, sizeof(struct dso_chain));
   so_loaded->dso->dloaded = 1;
   so_loaded->dso->dload_mode = mode;
   PROT_DATA(so_loaded->dso, sizeof(struct dso_chain));
  }
#endif
...
void dl_close(void *map) {
  ...
  /* TODO: so_chain is probably libc if it was moved to front and not main! 
   * Either compare with main so or move libc after main. */
...
/* somehow it does not find the symbol within the included sl_libc.h */
