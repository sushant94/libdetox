...
extern void REG_ARGS_STDCALL(3) signal_error(long errcode, const char *objname,
                                     const char *occasion,
...
unsigned long resolve_plt(dso *so, unsigned long offset, struct dso_chain *dso_objects) {
  ...
  *got_fixup =
    (unsigned long) resolve_symbol(so, so->string_table
                                   ...
                                   version, 1, 0, &sym_found, &so_found, dso_objects);
...
void *resolve_symbol(dso *so_start, const char *name,
                     ...
                     Elf32_Sym **sym_found, dso **so_found, struct dso_chain *dso_objects) {
  Elf32_Sym *sym = 0;
  dso *so = 0;
    ...
    for (so = gscope; so != 0; so = so->gs_next) {
      ...
      if (sym != 0 && sym->st_shndx != SHN_UNDEF) {
        ...
        *sym_found = sym;
        ...
        //void *origvalue = value;
        /* TODO/SECURITY ISSUE: this is exececuted outside TRuE! */
        /* 1. we need to make sure that the function called runs within TRuE */
        /* 2. we need to be able to verify CFTX to values returned by STT_GNU_IFUNC's */
        if (ELF32_ST_TYPE(sym->st_info) == STT_GNU_IFUNC) {
          /* GNU IFUNC extension: the function at the symbol address
             returns the real address */
          //sl_printf("Found %s, %d with value %x, st_info %d \n", name, sym->st_name, sym->st_value, sym->st_info);
          value = ((void *(*) (void))(value))();
          //sl_printf("Found %s, %d with value %x, st_info %d \n", name, sym->st_name, value, sym->st_info);
          #if defined(VERIFY_CFTX)
          /* We have a better solution: resolve STT_GNU_IFUNC symbols during load time! */
			//set_symbol_address(so->name, name, value, origvalue, dso_objects);
		  #endif
        }
        return value;
      }
...
unsigned long resolve() {
  ...
  */
  ...
  return resolve_plt(so, offset, NULL);
}
...
/* EXPORTED FUNCTIONS */
struct link_map * REG_ARGS_STDCALL(3) lookup_symbol_x
  ...
  if (undef_map == (struct link_map *) so_chain) {
    ...
    ret = resolve_symbol(0, undef_name, version, 1, 0, &sym_found, &so_found, NULL);
  ...
  *ref = sym_found;
  ...
  /* lookup_symbol_x will be called if dl_sym tries to resolve an dynamically loaded DSOs symbol,
   * we therefore need to add this symbol/address to the set of callbacks as this address might
   * be called or even passed around DSOs. Ideally we just allow calls from DSOs that called dl_sym,
   * but for now we add it as a DSO callback. TODO: improve? */
#if defined(VERIFY_CFTX)
	 if(sym_found != 0 && so_found != 0) {
		if (PTR_IN_REGION((so_found->base_addr+sym_found->st_value), so_found->text_addr, so_found->text_size)) {
						fbt_add_callback(so_found->dso, (void*)(so_found->base_addr+sym_found->st_value));
					}
	 }
#endif
...
/* TODO: handle different calling convention! It seems that libdl expects at the end a ret $0x4 instead of a plain ret! */
/* It seems that the glibc guys switched from cdecl calling convention to stdcall. */
long REG_ARGS_STDCALL(3) dl_addr(const void *address, Dl_info *info,
